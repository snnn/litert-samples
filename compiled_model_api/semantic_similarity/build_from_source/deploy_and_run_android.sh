...
readonly C_LIBRARY_LOCATION="${ROOT_DIR}/bazel-bin/external/litert_archive/litert/c"
# Target to build
readonly TARGET="//compiled_model_api/semantic_similarity/build_from_source:semantic_similarity"
...
readonly LOCAL_MODEL_DIR="${ROOT_DIR}/compiled_model_api/semantic_similarity/build_from_source/models"
# tokenizer filename
TOKENIZER_MODEL="${LOCAL_MODEL_DIR}/sentencepiece.model"
EMBEDDER_MODEL="${LOCAL_MODEL_DIR}/embeddinggemma-300M_seq256_mixed-precision.tflite"
...
build() {
  ...
  bazel build ${BUILD_FLAGS} \
      @litert_archive//litert/c:libLiteRt.so
...
push() {
  ...
  local local_binary_path="${ROOT_DIR}/bazel-bin/compiled_model_api/semantic_similarity/build_from_source/semantic_similarity"
